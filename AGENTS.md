# Constituição operacional dos agentes — SUNSET ERP
...
Use os agentes customizados em `.codex/agents/` conforme `docs/agents/01_AGENT_ROUTING_MATRIX.md`.
O agente orchestrator é responsável por decomposição e consolidação.
O agente code_explorer deve preceder mudanças em áreas desconhecidas.
O agente requirements_traceability deve validar cobertura antes de declarar conclusão.
O agente contradiction_researcher deve ser usado em decisões de alto impacto ou premissas frágeis.
O agente pr_reviewer deve ser independente do agente que implementou.
Agentes read-only não podem ser convertidos em implementadores durante a mesma execução.
Agentes especialistas não podem alterar decisões de produto.
Nenhum subagente pode criar outro subagente além da profundidade configurada.
Use no máximo quatro threads e reduza para duas em migrations ou contratos compartilhados.
Leia `docs/agents/00_AGENT_SYSTEM_OVERVIEW.md` antes do primeiro trabalho multiagente.
Leia `docs/agents/02_HANDOFF_PROTOCOL.md` antes de delegar.
Leia `docs/agents/03_EVIDENCE_AND_SOURCE_POLICY.md` antes de classificar risco.
Valide o pacote com `python3 scripts/validate-agent-pack.py` após qualquer alteração.

# BASE DE FONTES
As tags e URLs oficiais estão em `docs/agents/SOURCE_BASIS.md`.
As regras deste arquivo foram fundamentadas nas fontes registradas e no blueprint aprovado.

## Cursor Cloud specific instructions

Durable, non-obvious notes for running this monorepo in the Cloud Agent VM. Standard commands live in `README.md` and the `scripts` of each `package.json`; only the caveats below are non-obvious.

### Services and ports
- API (`apps/api`, NestJS/Fastify) → `3000`, routes prefixed `/v1`, health at `/health` and `/ready`.
- Web (`apps/web`, Next.js) → `3001`. Runs independently of the API; unauthenticated `/` redirects to `/app`, login form is at `/login`.
- Worker (`apps/worker`, NestJS) → `3002`; `/ready` performs a real Postgres connectivity check.
- Postgres 16 → `5432` (dev) and `5433` (test profile), via `docker-compose.yml`.

### Postgres runs in Docker; the daemon is NOT auto-started
- Docker is preinstalled but there is no init system: start the daemon yourself with `sudo dockerd` in a background/tmux session before `pnpm db:up`. Verify with `docker info`.
- A `docker-compose` (hyphen) shim at `/usr/local/bin/docker-compose` forwards to `docker compose`, so the repo's `pnpm db:up`/`db:down` work. The `ubuntu` user is in the `docker` group (effective in new shells once `dockerd` is running).
- Docker uses the `fuse-overlayfs` storage driver with the containerd-snapshotter feature disabled (`/etc/docker/daemon.json`); do not switch it to `overlay2` in this VM.

### Env and Prisma ordering caveats
- Copy `.env` from `.env.example` before running anything (`cp .env.example .env`).
- Prisma commands run from `packages/db` and do NOT auto-load the root `.env`. Pass `DATABASE_URL` (and `SEED_ADMIN_PASSWORD` for seed) explicitly in the environment when invoking them.
- pnpm ignores dependency build scripts by default, so `@prisma/client` is not generated by `pnpm install`. Run `pnpm --filter @sunset/db exec prisma generate` after install (the update script does this).
- Seed order matters: `prisma migrate deploy` → `prisma generate` → `prisma db seed` (the seed imports the generated client, so generate must precede it). Migrations create the `uuid_generate_v7()` DB function, so they must run before the app/seed.
- Seeded admin login: `admin@sunset.local` with password `SEED_ADMIN_PASSWORD` (holds the `admin` role with all 50 permissions).

### Known pre-existing breakage on `main` (not an environment problem)
- `pnpm test:ci` passes (Vitest transpiles without full type-checking). But `pnpm build`, `pnpm typecheck`, and `pnpm lint` currently FAIL on `main`, and `pnpm --filter api dev` does not start.
- Root cause is committed defects in `apps/api/src/inventory/`: `warehouse.module.ts` contains literal `\n` escape sequences (fatal syntax errors) and malformed provider objects referencing an unimported `PrismaSoDRepository`; `dto/inventory-sod.dto.ts` imports SoD command types that `@sunset/contracts` does not export; several inventory services have Prisma `null`→JSON (`NullableJsonNullValueInput`) type errors. `packages/ui` also has 4 Biome a11y lint errors.
- These are application-code bugs to be fixed through the normal slice/review process — do not assume the VM setup is broken when you see them.
