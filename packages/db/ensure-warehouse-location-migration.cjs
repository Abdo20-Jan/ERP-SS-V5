...
const dir = path.join(
  root,
  "prisma/migrations/20260723000000_create_warehouse_locations",
);
const file = path.join(dir, "migration.sql");
const sql = `-- Warehouse locations (PR-INVENTORY-01-S02)
...
if (!fs.existsSync(file)) {
  ...
  console.log("wrote warehouse location migration");
} else {
  console.log("warehouse location migration already present");
}
