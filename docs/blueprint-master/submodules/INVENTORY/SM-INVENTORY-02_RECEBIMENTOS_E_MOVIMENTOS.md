# SM-INVENTORY-02 — Recebimentos e movimentos

**Status:** `APPROVED_FOR_PLANNING_AND_EXECUTION`
**Módulo:** Estoque
**PR lógico:** `PR-INVENTORY-02`

## Resultado esperado

Entregar recebimentos e movimentos de forma integrada, auditável, idempotente e otimizada para operação diária.

## Funções aprovadas

1. Receber mercadoria de importação, compra, devolução ou transferência.
2. Criar movimentos de entrada, saída, transferência e reclassificação.
3. Vincular movimento a SKU, despacho, contêiner, posição e documento.
4. Controlar quantidade, unidade, data, responsável e evidência.
5. Registrar remito assinado e confirmação de descarga.
6. Impedir movimento duplicado ou saldo negativo não autorizado.
7. Executar movimentação em lote com pré-validação.
8. Manter razão de estoque append-only e auditável.
...
- Controller e 10 slices: `docs/requirements/pr-programs/INVENTORY/PR-INVENTORY-02/`
