# SM-COMEX-01 — Pedido internacional e produção

**Status:** `APPROVED_FOR_PLANNING_AND_EXECUTION`
**Módulo:** COMEX
**PR lógico:** `PR-COMEX-01`

## Resultado esperado

Entregar pedido internacional e produção de forma integrada, auditável, idempotente e otimizada para operação diária.

## Funções aprovadas

1. Criar o pedido internacional e suas linhas por SKU, quantidade, preço, moeda e Incoterm.
2. Vincular fornecedor, exportador, fabricante, proforma e condições de pagamento.
3. Executar workflow de aprovação, envio ao fornecedor, confirmação e alteração controlada.
4. Controlar cronograma de produção, datas prometidas, produção parcial e saldo pendente.
5. Registrar versões da proforma e impedir duplicidade do mesmo pedido/documento.
6. Importar itens em massa por Excel e reaproveitar o cadastro mestre de SKUs.
7. Exibir status, atrasos, próximas ações, responsáveis e exceções em uma única página.
8. Gerar eventos para previsão de caixa, documentos, embarque e auditoria.
...
- Controller e 10 slices: `docs/requirements/pr-programs/COMEX/PR-COMEX-01/`
