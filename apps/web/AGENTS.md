# Instruções locais — Web operacional

Estas regras se aplicam a `apps/web/` e descendentes.

# AGENTES RESPONSÁVEIS
Acione `ux_operations` para mudanças relevantes neste diretório.
Acione `commercial` para mudanças relevantes neste diretório.
Acione `testing_quality` para mudanças relevantes neste diretório.

# OBJETOS LOCAIS
Mapeie ownership e contrato de componentes React antes de alterar.
Não duplique componentes React se existir implementação compartilhada.
Inclua teste e documentação para mudança em componentes React.
Verifique impacto de segurança, dados e operação em componentes React.
Mapeie ownership e contrato de rotas antes de alterar.
Não duplique rotas se existir implementação compartilhada.
Inclua teste e documentação para mudança em rotas.
Verifique impacto de segurança, dados e operação em rotas.
...
Mapeie ownership e contrato de atalhos antes de alterar.
Não duplique atalhos se existir implementação compartilhada.
Inclua teste e documentação para mudança em atalhos.
Verifique impacto de segurança, dados e operação em atalhos.
Mapeie ownership e contrato de acessibilidade antes de alterar.
Não duplique acessibilidade se existir implementação compartilhada.
Inclua teste e documentação para mudança em acessibilidade.
Verifique impacto de segurança, dados e operação em acessibilidade.
...
Local-1. Preserve compatibilidade de componentes React.
Local-1. Não altere componentes React fora do slice.
Local-1. Registre evidência de componentes React.
Local-1. Revise performance e falha de componentes React.
Local-1. Documente rollback de componentes React.
Local-2. Preserve compatibilidade de rotas.
Local-2. Não altere rotas fora do slice.
Local-2. Registre evidência de rotas.
Local-2. Revise performance e falha de rotas.
Local-2. Documente rollback de rotas.
...
Local-5. Preserve compatibilidade de atalhos.
Local-5. Não altere atalhos fora do slice.
Local-5. Registre evidência de atalhos.
Local-5. Revise performance e falha de atalhos.
Local-5. Documente rollback de atalhos.
Local-6. Preserve compatibilidade de acessibilidade.
Local-6. Não altere acessibilidade fora do slice.
Local-6. Registre evidência de acessibilidade.
Local-6. Revise performance e falha de acessibilidade.
Local-6. Documente rollback de acessibilidade.
